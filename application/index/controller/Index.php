...
class Index
{
    ...
    public function sendImg(){
        ...
        if($_FILES['upfile']['tmp_name'])
        {
            ...
            if($tmp) $result = $this->uploadFile($tmp);
            dump($result);
            die();
        }
        else
...
//        return $result;
    ...
    public function hello($id=1)
    {
        ...
        try{
            /**
             *   ["timestamp"] =&gt; int(1573196483199)
            ["status"] =&gt; int(404)
            ["error"] =&gt; string(9) "Not Found"
            ["message"] =&gt; string(20) "No message available"
            ["path"] =&gt; string(1) "/"
             */
            $tmp = $_FILES['upfile']['tmp_name'];
            if($tmp){
...
//        $header = array('enctype:multipart/form-data', $token, "restrict-access:true");
        $header = array('enctype:multipart/form-data');
